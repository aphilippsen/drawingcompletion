# Training of a grid of networks with different reliance of prior and sensory input
...
default_hyp_prior = 1
# default_sigma_input defined below
hyp_prior_factors = [1000,1,0.001,1000,1,0.001,1000,1,0.001] #[1000, 1, 0.001, 0.01, 0.1, 10, 100]
sigma_input_factors = [1000,1000,1000,1,1,1,0.001,0.001,0.001]
...
ext_var_proactive = np.inf

runs = len(hyp_prior_factors)

# all hyp_prior_factors conditions use the same initial weights
...
for r in range(runs):
    ...
    if len(hyp_prior_factors) > 0:
        hyp_prior_factor = hyp_prior_factors[r]
    if len(sigma_input_factors) > 0:
        sigma_input_factor = sigma_input_factors[r]



    final_save_dir = os.path.join(save_dir, str(hyp_prior_factor) + "_" + str(sigma_input_factor))
    ...
    sensor_noise_vec = xp.ones((x_train.shape)) * sensor_noise
    # Add noise to training data
    if gpu_id >= 0:
        x_train = cuda.to_gpu(x_train) + xp.sqrt(sensor_noise_vec) * xp.random.randn(x_train.shape[0], x_train.shape[1])
        x_train = xp.float32(cuda.to_cpu(x_train))
    else:
        x_train += xp.sqrt(sensor_noise_vec) * xp.random.randn(x_train.shape[0], x_train.shape[1])
    ...
    default_sigma_input = np.max([training_trajectory_variance, sensor_noise])
    ...
    p.set_network_type('SCTRNN', {'num_io':x_train.shape[1]/num_timesteps, 'num_c':num_neurons, 'lr':lrate, 'num_classes': num_classes,
       ...
       'hyp_prior':default_hyp_prior*hyp_prior_factor,
       'external_signal_variance':default_sigma_input*sigma_input_factor})

    with open(os.path.join(final_save_dir,"info.txt"),'w') as f:
       f.write(p.get_parameter_string())
       f.write("\n")
       f.write("Variance added to training data: " + str(sensor_noise) + "\n")
       f.write("\n")
       f.write("\n"+experiment_info+"\n")
    ...
    res, resv, resm, pe, wpe, respos = model.generate(model.initial_states.W.array, num_timesteps, external_input = xp.copy(xp.asarray(x_train[:test_batch_size,:])), add_variance_to_output = 0, external_signal_variance = p.external_signal_variance)
    results = cuda.to_cpu(res)
    ...
    for epoch in range(1, p.epochs + 1):
        ...
        if epoch%save_interval == 1 or epoch == p.epochs:
           ...
           res, resv, resm, pe, wpe, u_h_history, respos = model.generate(model.initial_states.W.array, num_timesteps, external_input = xp.copy(xp.asarray(x_train[:test_batch_size,:])), add_variance_to_output = 0, additional_output='activations', external_signal_variance = p.external_signal_variance)
           results = cuda.to_cpu(res)
