...
condition_directories = ['0.001', '0.01', '0.1', '1', '10', '100', '1000']
# which hyp_prior condition to use for testing (corresponding to condition_directories)
test_hyp_priors = [0.001, 0.01, 0.1, 1, 10, 100, 1000]
...
# if True, already existing result folders will be overridden
...
for current_r in range(len(run_directories)):
    ...
    for current_c in range(len(condition_directories)):
        ...
        # use the corresponding test hyp prior
        hyp_prior = test_hyp_priors[current_c]
        params, model = load_network(network_dir, model_filename='network-epoch-best.npz')
        ...
        if os.path.isfile(os.path.join(results_dir, 'final-res_hyp-' + str(hyp_prior) + '_mode-' + str(is_selection_mode) + '.npy')) and not regenerate_results:
